...
from PySide6.QtGui import QCursor, QTextFrameFormat, QTextCursor, QTextDocument
from PySide6.QtGui import QFont, QTextTableFormat, QTextLength, QPageLayout
from PySide6.QtWidgets import QAbstractItemView, QApplication, QCheckBox, QMainWindow
from PySide6.QtWidgets import QMessageBox, QStyle, QWidget, QDialog
...
class Jan_Compra_Prod(QWidget, Ui_wind_add_compra):
    ...
    def confirma_compra_prod(self):
        ...
        if window.lim_min is None:
            ComprasRepo.my_insert(
                self, pcidprod=idprod.ID_PROD, pclimmin=float(quantidade)
            )
        else:
            ComprasRepo.my_update(
                self, pcidprod=idprod.ID_PROD, pclimmin=float(quantidade)
            )
...
class Jan_Entr_Prod(QWidget, Ui_wind_entr_prod):
    ...
    def confirma_entr_prod(self):
        ...
        if not data or not quantidade:
            window.show_mensagem(">> Data e/ou Quantidade não podem ser vazios!")
            return
        saldo = float(self.cp_saldo.text())
        saldo = saldo + float(quantidade)
        idprod = EstoqueRepo.my_select_one(self, **{"COD_FABR": self.cp_codfabr.text()})
        EntradasRepo.my_insert(
            self, indata=data, inquant=float(quantidade), inidprod=idprod.ID_PROD
        )
...
class Jan_Saida_Prod(QWidget, Ui_wind_saida_prod):
    ...
    def confirma_saida_prod(self):
        ...
        if not data or not quantidade or not destino:
            window.show_mensagem(">> Todos os campos devem ser preenchidos!")
            return
        saldo = float(self.cp_saldo.text())
        if float(quantidade) > saldo:
            window.show_mensagem(
                ">> Quantidado de Saída maior que o Saldo não permitida!"
            )
            return
        saldo = saldo - float(quantidade)
        idprod = EstoqueRepo.my_select_one(self, **{"COD_FABR": self.cp_codfabr.text()})
        SaidasRepo.my_insert(
            self,
            outdata=data,
            outquant=float(quantidade),
            outidprod=idprod.ID_PROD,
            outdestino=destino,
        )
        EstoqueRepo.my_updsaldo(self, codfabr=self.cp_codfabr.text(), saldo=saldo)
...
class Jan_Alt_Prod(QWidget, Ui_wind_alt_prod):
    ...
    def confirma_alt_prod(self):
        ...
        EstoqueRepo.my_update(
            self,
            codfabr_old=codfabr_old,
            codint=codint,
            codfabr=codfabr,
            produto=descprod,
...
class Jan_Add_Forn(QWidget, Ui_wind_add_forn):
    ...
    def confirma_forn(self):
        nomefabr = self.le_nomeforn.text()
        if not nomefabr:
            window.show_mensagem(">> Nome do Fabricante não pode ser vazio!")
            return
...
class MainWindow(QMainWindow, Ui_MainWindow, Ui_wind_add_prod, Ui_wind_add_forn):
    def __init__(self):
    ...
    def carrega_dados(self, tabela, tab_widget, ordem=None):
        if tabela == "ESTOQUE":
            ...
            self.model = TableModel(
                ...
                _icon_wrn=self._icon_wrn,
                _icon_ok=self._icon_ok,
            )
    ...
    def excluir(self):
        ...
        if aba == 0:  # ! EXCLUIR PRODUTOS
            ...
            if len(registro) <= 0:
                self.show_mensagem(">> Favor selecionar um item para exclusão!")
                return
            msg, sim, nao = self.msg_Box(
                "Exclusão de Produtos", "Confirma exclusão do Produto?"
            )
            ...
            if len(registro) <= 0:
                ...
                return
            msg, sim, nao = self.msg_Box(
                "Exclusão de Fabricante", "Confirma exclusão do fabricante?"
            )
            if msg.clickedButton() == nao:
                return
            if msg.clickedButton() == sim:
    ...
    def alterar(self):
        ...
        if aba == 0:
            ...
            if len(codint) <= 0:
                self.show_mensagem(">> Favor selecionar um item para alteração!")
                return
    ...
    def saida_excl(self):
        ...
        if not codfabr:
            ...
            return
        quantidade = float(
            self.tw_saidas.selectionModel().selectedRows(column=1)[0].data()
        )
    ...
    def saida(self):
        ...
        if aba == 0:  # ! SAÍDA DE PRODUTOS
            ...
            saldo = self.tw_prod.selectionModel().selectedRows(column=4)
            if len(codint) <= 0:
                self.show_mensagem(">> Favor selecionar um item para efetuar a saída!")
                return
    ...
    def compras(self):
        ...
        if aba == 0:  # ! PONTO DE COMPRAS
            ...
            self.lim_min = ComprasRepo.my_select_one(
                self, **{"PC_IDPROD": idprod.ID_PROD}
            )
    ...
    def pesquisa(self):
        ...
        if pagina == 0:
            ...
            if aba == 0:
                ...
                self.objPesquisa = EstoqueRepo.my_pesquisa(
                    self, **{"coluna": ObjPesquisa}
                )
                if len(self.objPesquisa) > 0:
                    self.show_mensagem(
                        ">> Registros encontrados = {}".format(len(self.objPesquisa))
                    )
                else:
            ...
            if aba == 1:
                ObjPesquisa = self.le_cad_pesq.text()
                ObjPesquisa = "%" + ObjPesquisa + "%"
                self.objPesquisa = FabricantesRepo.my_pesquisa(
                    self, **{"coluna": ObjPesquisa}
                )
                if len(self.objPesquisa) > 0:
                    self.show_mensagem(
        ...
        if self.cb_classif.isChecked():
            if aba == 0:
                ...
                if comando == "ID_FABR":
                    ...
                    if self.cb_decresc.isChecked():
                        self.ordem = EstoqueRepo.my_orderby(
                            self, descendente=True, **{"coluna": comando}
                        )
                    else:
        ...
        if self.cb_classif_forn.isChecked():
            if aba == 1:
                ...
                if self.cb_decresc_forn.isChecked():
                    self.ordem = FabricantesRepo.my_orderby(
                        self, descendente=True, **{"coluna": comando}
                    )
                else:
    ...
    def handlePaintRequest(self, printer):
        ...
        if pagina == 0:
            ...
            if aba == 0:
                ...
                cursor.insertHtml(
                    '<center><span style="color:#0000ff;"><h1>Relatório do Estoque<h1></span>'
                )
            if aba == 1:
                ...
                cursor.insertHtml(
                    '<center><span style="color:#0000ff;"><h1>Relatório de Fabricantes<h1></span>'
                )
            ...
            cursor.insertHtml(
                '<center><span style="color:#0000ff;"><h1>Relatório de Entradas<h1></span>'
            )
        elif pagina == 2:
            model = self.tw_saidas.model()
            cabec = ["Data Saída", "Quant.", "Cod. Fabricante", "Produto", "Destino"]
            constraints = [
                QTextLength(QTextLength.FixedLength, 75),
                QTextLength(QTextLength.FixedLength, 50),
            ...
            cursor.insertHtml(
                '<center><span style="color:#0000ff;"><h1>Relatório de Saídas<h1></span>'
            )
        elif pagina == 3:
            ...
            cursor.insertHtml(
                '<center><span style="color:#0000ff;"><h1>Relatório do Ponto de Compra<h1></span>'
            )
    ...
    def muda_tema(self, t):
        apply_stylesheet(app, theme=t, invert_secondary=False, extra=self.extra)
    ...
    def left_menu(self):
        ...
        self.animation = QtCore.QPropertyAnimation(self.fr_esquerda, b"maximumWidth")
    ...
    def slots(self):
        ...
        self.pb_entradas.clicked.connect(
            lambda: self.sw_paginas.setCurrentWidget(self.Entradas)
        )
        ...
        self.pb_compras.clicked.connect(
        ...
        self.cb_classif.stateChanged.connect(
            lambda: self.classificacao(self.cb_classif)
        )
        self.cb_decresc.stateChanged.connect(
            lambda: self.classificacao(self.cb_decresc)
        )
        ...
        self.cb_classif_forn.stateChanged.connect(
            lambda: self.classificacao(self.cb_classif_forn)
        )
        self.cb_decresc_forn.stateChanged.connect(
            lambda: self.classificacao(self.cb_decresc_forn)
        )
    ...
    def msg_Box(self, titulo, mensagem):
        ...
        msg.setStyleSheet(
            "QLabel{font-size: 20px;} QPushButton{ width:100px; font-size: 18px; }"
        )
    ...
    def msg_Box_cb(self, titulo, mensagem, info, cb_txt):
        ...
        cb = QCheckBox()
        msg.setStyleSheet(
            "QLabel{font-size: 18px;} QPushButton{ width:100px; font-size: 18px; }"
        )
        msg.setCursor(QCursor(Qt.PointingHandCursor))
        msg.setWindowTitle(titulo)
        msg.setText(mensagem)
